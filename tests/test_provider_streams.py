...
from character_chat_local.providers import (
    GeminiProvider,
    OllamaProvider,
...
def transport(monkeypatch):
    ...
    def install(body):
        ...
        monkeypatch.setattr(
            "character_chat_local.providers.httpx.AsyncClient",
...
def provider(kind):
    ...
    return OpenAICompatibleProvider(
        provider_id=kind, api_key="test-key", base_url="https://api.example"
    )
...
def response_body(kind, finish=True):
    ...
    if kind == "gemini":
        data = {
            "content": {
                "parts": [{"text": "思考", "thought": True}, {"text": "こんにちは"}]
            }
        }
        if finish:
...
async def test_stream_protocol_and_hidden_thoughts(kind, transport):
    ...
    result = await _collect(
        provider(kind), "small", [ChatMessage(role="user", content="hi")], 0.8
    )
