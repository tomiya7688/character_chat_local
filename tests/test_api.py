...
def client(tmp_path, provider):
    app = create_app(database_path=tmp_path / "api.db", registry=ProviderRegistry([provider]))
    with TestClient(app, base_url="http://127.0.0.1") as client:
        yield client


def new_conversation(client):
    character = client.post("/characters", json={"name": "ミカ", "first_person": "私"}).json()
...
def send(client, conversation, **kwargs):
    return client.post(f"/conversations/{conversation}/chat", json={
        "provider": "fake", "model": "small", "user_input": "こんにちは", **kwargs,
    })
...
def test_character_model_selection_chat_history_and_update(client, provider):
    ...
    page2 = client.get(f"/conversations/{conversation}/messages?after={messages[-1]['position']}").json()
    ...
    assert client.put(f"/characters/{character['id']}", json=character).status_code == 200
    assert client.get(f"/characters/{character['id']}").json()["speech_style"] == ["ゆっくり"]
...
@pytest.mark.parametrize("payload", [{"user_input": "  "}, {"temperature": -1}, {"user_input": "x" * 4001}, {"history": []}])
...
def test_nonexistent_and_mismatched_resources(client):
    ...
    assert client.post("/conversations", json={"character_id": "missing"}).status_code == 404
    assert client.get("/characters/missing").status_code == 404
    assert client.post("/characters", json=character).status_code == 409
    assert client.post(f"/characters/{character['id']}/memories", json={"character_id": "other", "type": "canon", "content": "test"}).status_code == 422
    assert client.get(f"/conversations/{conversation}/summary").json()["strategy"] == "extractive-v1"
...
def test_memory_source_must_belong_to_character(client):
    ...
    payload = {"character_id": character["id"], "type": "user_fact", "content": "明示情報", "source_message_id": source}
    assert client.post(f"/characters/{character['id']}/memories", json=payload).status_code == 201
    payload["character_id"] = other["id"]
    assert client.post(f"/characters/{other['id']}/memories", json=payload).status_code == 422
...
def test_local_origin_host_and_fetch_metadata_boundaries(client):
    ...
    assert client.get("/characters", headers={"Sec-Fetch-Site": "cross-site"}).status_code == 403
    assert client.get("/characters", headers={"Origin": "http://127.0.0.1"}).status_code == 200


def test_optional_token_and_import_without_database_side_effect(tmp_path):
    path = tmp_path / "api.db"
    app = create_app(database_path=path, registry=ProviderRegistry([]), api_token="test-token")
    assert not path.exists()
    with TestClient(app, base_url="http://localhost") as client:
        assert client.get("/health").status_code == 200
        assert client.get("/characters").status_code == 401
        assert client.get("/characters", headers={"Authorization": "Bearer test-token"}).status_code == 200
...
async def test_parallel_send_is_rejected_without_extra_generation(tmp_path):
    ...
    class Blocking(ScriptedProvider):
        async def stream_chat(self, **kwargs):
            ...
            yield "こんにちは。"
    provider = Blocking()
    app = create_app(database_path=tmp_path / "parallel.db", registry=ProviderRegistry([provider]))
    async with app.router.lifespan_context(app):
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://127.0.0.1") as client:
            character = (await client.post("/characters", json={"name": "ミカ"})).json()
            conversation = (await client.post("/conversations", json={"character_id": character["id"]})).json()["id"]
...
def test_explicit_dev_origin_and_token_preflight(tmp_path):
    app = create_app(database_path=tmp_path / "cors.db", registry=ProviderRegistry([]), allowed_origins=("http://localhost:5173",), api_token="token")
    with TestClient(app, base_url="http://127.0.0.1") as client:
        headers = {"Origin": "http://localhost:5173", "Sec-Fetch-Site": "cross-site"}
        preflight = client.options("/characters", headers={**headers, "Access-Control-Request-Method": "POST", "Access-Control-Request-Headers": "Authorization,Content-Type"})
        assert preflight.status_code == 200
        assert client.get("/characters", headers=headers).status_code == 401
        response = client.get("/characters", headers={**headers, "Authorization": "Bearer token"})
        assert response.status_code == 200
