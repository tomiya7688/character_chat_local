...
def test_1000_roundtrips_restart_model_switch_and_provenance(tmp_path):
    ...
    for phase in range(2):
        ...
        with TestClient(app, base_url="http://127.0.0.1") as client:
            if phase == 0:
                character = client.post("/characters", json={"name": "ミカ", "lore": ["海辺の町"]}).json()
                character_id = character["id"]
                conversation = client.post("/conversations", json={"character_id": character_id}).json()["id"]
            for turn in range(phase * 500, (phase + 1) * 500):
                text = "私の名前はカドカ。約束を覚えてね" if turn == 0 else f"会話 {turn}"
                response = client.post(f"/conversations/{conversation}/chat", json={
            ...
            other = client.post("/conversations", json={"character_id": character_id}).json()["id"]
    ...
    with storage.session() as db:
        ...
        assert db.execute("SELECT count(*) FROM messages WHERE conversation_id=?", (conversation,)).fetchone()[0] == 2000
        assert db.execute("SELECT count(*) FROM response_evaluations").fetchone()[0] == 1000
        for entry in summary.entries:
            row = db.execute("SELECT content, role FROM messages WHERE id=?", (entry.source_message_id,)).fetchone()
            assert entry.excerpt == row["content"][:160] and entry.role == row["role"]
        rows = db.execute("SELECT model, count(*) AS count FROM messages WHERE role='assistant' GROUP BY model").fetchall()
        assert {row["model"]: row["count"] for row in rows} == {"tiny": 500, "other-model": 500}
    assert [p.count for p in providers] == [500, 500]
    report = {"turns": 1000, "messages": 2000, "covered_messages": summary.covered_messages,
              "max_prompt_bytes": max(p.largest_prompt for p in providers), "real_inference": False}
    (tmp_path / "endurance-result.json").write_text(json.dumps(report), encoding="utf-8")
